...
// TestBashTool_ReportResult_Precedence pins the post-run precedence ladder
// exactly: cancelled beats timeout beats tooLarge. A turn that is both timed
// out and cancelled must surface "cancelled", never the timeout bubble.
func TestBashTool_ReportResult_Precedence(t *testing.T) {
	tool := &BashTool{}
	p := &bashParams{Command: "sleep 30", Timeout: 60}
	...
	tests := []struct {
		...
		{"cancelled alone", true, false, false, "cancelled"},
		{"cancelled beats timeout", true, true, false, "cancelled"},
		{"cancelled beats timeout+tooLarge", true, true, true, "cancelled"},
		{"timeout beats tooLarge", false, true, true, "timeout"},
		{"tooLarge alone", false, false, true, "output_too_large"},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.cancelled {
				...
				cancel()
				ctx = cctx // already-cancelled turn
			}
			_, err := tool.reportResult(ctx, p, "", []byte("output"), time.Second, tc.timedOut, tc.tooLarge, nil)
			var te *internal.ToolError
			if !errors.As(err, &te) {
				t.Fatalf("expected ToolError, got %v", err)
			}
			if te.Type != tc.wantType {
				t.Errorf("precedence ladder broken: want %q, got %q", tc.wantType, te.Type)
			}
		})
	}
}

// TestBashTool_ReportResult_HintPrependPreserved pins that the non-blocking
// file-edit hint is prepended (not replaced) on the success path.
func TestBashTool_ReportResult_HintPrependPreserved(t *testing.T) {
	tool := &BashTool{}
	p := &bashParams{Command: "echo hi > out.txt"}
	out, err := tool.reportResult(context.Background(), p, "HINT\n", []byte("hi"), 10*time.Millisecond, false, false, nil)
	if err != nil {
		t.Fatalf("expected success, got %v", err)
	}
	if !strings.HasPrefix(out, "HINT\n") {
		t.Errorf("file-edit hint not prepended, got: %q", out)
	}
	if !strings.Contains(out, "Duration:") {
		t.Errorf("duration footer lost, got: %q", out)
	}
}
