...
func writeModelRegistration(f *os.File, id string, m ModelsDevModel, pm ProviderMapping) int {
	ctxWindow := intOrDefault(m.Context, 4096)
	maxTokens := intOrDefault(m.OutputLimit, 4096)
	inputCost := floatOrDefault(m.InputCost)
	outputCost := floatOrDefault(m.OutputCost)
	cacheRead := floatOrDefault(m.CacheRead)
	cacheWrite := floatOrDefault(m.CacheWrite)
