...
| TL4 | **No hook-dialect compatibility layer.** Goa's hooks are its own 4-event format (`beforeTool/afterTool/sessionStart/sessionEnd`). Users with an existing Claude Code `hooks.json` (or Codex hooks) must rewrite. dsh runs the CC/Codex command-hook subsets on its canonical interception points, with env substitution and most-restrictive merge. | `internal/hooks/hooks.go:22-42` (goa-native events only) | `packages/hooks/hooks-claude-code/README.md`, `packages/hooks/hooks-codex/`, shared `packages/hooks/hook-protocol/` | **Low** |
...
### P17 — Hook-dialect compatibility (TL4) — LOW · effort M
