# Bug and feature Archive — 2026-08-30

Completed entries moved here from bugs.md per guideline 4 (all items below are
fixed/closed or historical working evidence; bugs.md holds only the guideline
and the open "# To fix" list from this date on).


## BUG: Muse Spark on OpenCode needs the OpenAI Responses endpoint — per-model catalog `api:` pins were dead on the main resolve path and responses-API models got a `/chat/completions` URL (2026-08-30)

**Observed:** OpenCode serves its Muse Spark family through the OpenAI **Responses** API — their docs list `Muse Spark 1.2` / `muse-spark-1.2` at `https://opencode.ai/zen/v1/responses` with npm `@ai-sdk/openai` — while every other opencode model (and the provider entry models.dev publishes) is `@ai-sdk/openai-compatible` chat completions at `https://opencode.ai/zen/v1`. Goa cataloged `muse-spark-1.2` as `openai-completions` under provider `opencode` (inherited from the provider-level mapping), so selecting it sent a chat-completions body to a gateway route that only serves `/responses` — instant provider error. Adding the model via the picker or `/config:add` had no way to express the per-model surface without hand-editing YAML.

**Root cause (three colluding defects in the model→wire plumbing):**

1. **No per-model API data for Muse Spark.** models.dev carries only provider-level `npm`; the embedded snapshot's opencode entries say `@ai-sdk/openai-compatible` for ALL models, so the catalog had no place where Muse Spark's Responses surface could be expressed (the per-model override mechanism in `internal/agentic/provider/models/model_overrides.yaml` supports an `api:` field, but nothing had set it).
2. **`mergeRegistryModel` discarded the registry model's API** (`provider/manager_fallback.go`): it unconditionally assigned `mdl.Api = api` from `inferEffectiveProviderAPI(pCfg, mCfg)` — the provider identity (or the model-config `api`) — so any per-model `api:` pin in `model_overrides.yaml` was DEAD on the main resolve path (it only survived on the sub-agent role path, `resolveModelByName`). Latent proof: `gpt-4o`/`gpt-5.5` are pinned `api: openai-completions` in the YAML, yet resolved as `openai-responses` on the openai preset (the opposite direction of the same defect).
3. **Responses-API models got a `/chat/completions` BaseURL.** `needsChatCompletionsSuffix` returned true for `openai-responses` (and `azure-openai-responses`), so `setModelBaseURL` appended `/chat/completions` — but the generic protocol runtime (`internal/agentic/provider/runtime.go` `resolveURL`) POSTs `model.BaseURL` VERBATIM (only Codex gets normalized to `{base}/codex/responses`). Net effect: the responses protocol body was POSTed to `…/chat/completions`. The role path had the same shape via its local `resolveURL` closure. Same-ID models from aggregators made the role path worse: `GetModel("muse-spark-1.2")` returns the GLOBAL first-wins index entry, which belongs to `llmgateway` — an entry from a different provider.

**Requirement:** the model carries an API flag at TWO levels — catalog level (`model_overrides.yaml` per-(provider, model) `api:`, prefix IDs supported) and user level (`ModelConfig.api`, already existing, must keep winning over everything). A catalog pin applies ONLY when the registry entry belongs to the resolved provider identity (Google's gemma entry resolved on LM Studio must stay `openai-completions` — the serving provider owns the wire surface). An `openai-responses`/`azure-openai-responses` model's BaseURL must be the full `{endpoint}/responses` route. All Muse Spark models on both OpenCode gateways (`opencode`: `muse-spark-1.2`, `muse-spark-1.2-contributor-free`; `opencode-go`: `muse-spark-1.2-contributor`) must resolve `openai-responses` at `https://opencode.ai/zen/{,go/}v1/responses` without any user configuration; every other opencode model stays byte-identical (`/chat/completions`).

**Fix plan:**
- `internal/agentic/provider/models/model_overrides.yaml`: prefix entries `id: "muse-spark-"` for providers `opencode` and `opencode-go` with `api: openai-responses` (everything else inherits from models.dev via `applyOverride`).
- `provider/manager_fallback.go` `mergeRegistryModel`: honor the registry model's `Api` when the model config does not state one AND the registry entry's provider matches the resolved identity (`m.Provider == prov` — the borrowed-entry guard).
- `provider/manager.go`: replace `needsChatCompletionsSuffix` with `modelEndpointURL(api, endpoint)` — `/chat/completions` suffix for `openai-completions`, `ResponsesEndpoint` (`{base}/responses`, idempotent) for `openai-responses` AND `azure-openai-responses` (their protocols POST the URL verbatim), endpoint unchanged otherwise (Anthropic/Google/Bedrock/Mistral own their path; Codex normalizes at request time).
- `provider/manager_identity.go` `resolveModelByName`: provider-exact registry lookup first (`GetModelForProvider`, global index only as fallback — the aggregator-entry bug), then the same provider-match rule; the later fallback branches reuse the once-resolved identity.

**Required tests/UT coverage (red before the fix):**
- Registry (`internal/agentic/provider/models/model_overrides_muse_test.go`): all three muse-spark IDs on their providers resolve `openai-responses` with inherited ContextWindow/Reasoning/Provider; `deepseek-v4-flash` (opencode) and `glm-5` (opencode-go) stay `openai-completions`.
- Main path (`provider/manager_muse_responses_test.go`): `ResolveActiveModel` for the three muse models → responses API + `{endpoint}/responses` BaseURL; `deepseek-v4-flash` unchanged; explicit `ModelConfig.api` wins in BOTH directions (force responses onto deepseek, force completions back onto muse); plain-OpenAI `gpt-5` → `https://api.openai.com/v1/responses`; `gpt-4o`/`gpt-5.5` completions pins restored to `/chat/completions`.
- Role path: `ResolveModelForProvider("opencode", muse)` → responses + `/responses` URL; borrowed gemma entry on LM Studio → `openai-completions`.
- URL helper (`provider/manager_endpoint_url_test.go`): `modelEndpointURL` table — completions suffix + idempotency, responses suffix + idempotency, azure, codex passthrough, Anthropic/Google passthrough, empty endpoint.
...
**Root cause (plain bool cannot distinguish "key absent" from "explicit false" — the auto_save_model bug shape, again):** `ExecutionConfig.LoopAutoResume` (`config/config_types.go`) is a plain `bool`, and `mergeExecution` (`config/config_merge.go`) copies it UNCONDITIONALLY: `dst.LoopAutoResume = src.LoopAutoResume`. Every cascade layer is built by `yaml.Unmarshal(&Config{})`, so a layer whose file OMITS `loop_auto_resume` still carries `LoopAutoResume == false` (zero value) — indistinguishable from an explicit `loop_auto_resume: false`. The project layer `/Users/muaddib/dev/goa/.goa/config.yaml` is a stale pre-field-scoped full-struct dump (its `execution:` section was marshaled before `SaveProjectConfig` became field-scoped in bb89bcc and before the feature existed, so it has every execution key EXCEPT `loop_auto_resume`). The project layer outranks home, so its unstated `false` silently overwrites home's `true` on every load. The sibling loop-detection toggles (`DisableThinkingLoopDetection`/`DisableToolLoopDetection`/`DisableStreamLoopDetection`/`DisableThinkingStallDetection`) are already tri-state `*bool` with nil-gated merges for exactly this reason; `LoopAutoResume` was added (5e73333) as a plain bool, reintroducing the defect. Two tests written for the feature pin the WRONG half of the contract: `TestMergeExecution_LoopAutoResumeSurvivesCascade` only checks the true-in-higher-layer direction (which unconditional copy passes), and nothing covers lower-layer-true + higher-layer-omits.

**Requirement:** `execution.loop_auto_resume` must be tri-state across the cascade: nil = inherit the lower layer, embedded default `false`; an explicit `true`/`false` at ANY layer wins over lower layers. Toggling on via the menu or `/config:set` must survive a restart regardless of what a stale higher-layer file omits; an explicit `loop_auto_resume: false` at a higher layer must still win over a lower layer's `true` (deliberate opt-out).

**Fix plan:**
- `config/config_types.go`: `LoopAutoResume` → `*bool` (`yaml:"loop_auto_resume,omitempty"`, tri-state doc mirroring the Disable*LoopDetection siblings). Embedded `default.yaml` already states `loop_auto_resume: false`, so the cascade base resolves to off — behavior-neutral default.
- `config/config_merge.go` `mergeExecution`: copy only when stated — nil-gated via a `mergeBoolPtrIfSet` helper (mirrors `mergeIntIfSet`; also applied to the sibling `AutoSaveModel` gate to keep `mergeExecution` within the gocyclo ≤12 budget).
- Readers: `core/agentmanager_loop.go` (auto-resume arm) and `core/commands/config.go` (`loopAutoResumeStatusLabel`, `settingLoopAutoResume`) read through a nil-safe resolver (`LoopAutoResumeEnabled()` or local nil check, matching the established tri-state family); the menu toggle and `/config:set` setter materialize an explicit value (`setBoolPtr`).
- `core/agentmanager_loopresume_test.go` and other readers' tests updated to materialize the pointer.

**Required tests/UT coverage (red before the fix):**
- Cascade (`config`): home states `loop_auto_resume: true`, project layer omits the key → merged TRUE (today: clobbered to false — the reported bug). Home true + project explicit `false` → FALSE (explicit higher-layer opt-out still wins). Home true + project explicit `true` → TRUE. Nothing anywhere → FALSE (embedded default). Existing true-direction merge test keeps passing.
- Menu/CLI: toggling via `settingLoopAutoResume`/`applyConfigSet("execution.loop_auto_resume", "true")` materializes an explicit `*bool` on the live config and persists; the status label reflects the resolved value.

**Implemented (2026-08-29).** *(see fix commit)* `config/config_types.go`: `LoopAutoResume` is now `*bool` (`loop_auto_resume,omitempty`, tri-state doc) with resolver `(ExecutionConfig).LoopAutoResumeEnabled()` (nil → false) beside the other feature resolvers. `config/config_merge.go`: `mergeExecution` copies only when the source layer states the key, via the new `mergeBoolPtrIfSet(&dst.LoopAutoResume, src.LoopAutoResume)` helper (the same helper now also carries the `AutoSaveModel` gate, keeping `mergeExecution` under the gocyclo ≤12 budget). Readers switched to the resolver: `core/agentmanager_loop.go` auto-resume arm; `core/commands/config.go` `loopAutoResumeStatusLabel`/`settingLoopAutoResume` (toggle now writes through `applySet` which materializes the explicit value); `/config:set execution.loop_auto_resume` uses `setBoolPtr`. Tests written RED first and GREEN after: `config/config_loopresume_test.go` gains the cascade matrix (lower-true/higher-omits → true; higher-explicit-false over lower-true → false; both-explicit → true; embedded-only → false) plus tri-state merge fidelity; reader tests updated for the pointer type. Validation: `go vet ./...` clean; `go test -count=1 -race -cover ./...` PASS (0 FAILs); gocognit `-over 15` / gocyclo `-over 12` on touched files zero findings; gofmt clean.


## BUG: TUI renders only a couple of mascot/logo lines then collapses — stderr-tee SetNonblock poisons the shared stdout open file description (2026-08-27) [FIXED]

**Observed:** on startup the TUI shows only the first couple of lines of the mascot/logo, then nothing renders; after a while a partial bubble appears but nothing more. The whole differential UI never draws.

**Root cause (commit d7c129d, the stderr-tee deadlock fix):** `teeStderr` (`internal/app/crash_log_unix.go`) called `unix.SetNonblock(saved, true)` on `saved = unix.Dup(unix.Stderr)` to make the terminal echo non-blocking. But `O_NONBLOCK` is a flag on the **open file description**, not the descriptor — and in a real terminal session the shell `dup2`s the PTY slave onto fds 0, 1 AND 2, so all three (and any `dup` of them, including `saved`) share ONE open file description. `SetNonblock(saved, true)` therefore silently flipped **stdout (fd 1)** to non-blocking for the entire session. `setupCrashLog` runs at the very top of `Main()` — before any frame — so from the first frame on, every multi-KB TUI frame write via `os.Stdout.Write` (ProcessTerminal.Write → `os.Stdout.Write`, whose error the render loop drops) short-wrote or failed with `EAGAIN` (Go treats a char device as a blocking fd, so `EAGAIN` surfaces as a plain error, not a retry). The screen showed exactly the truncated first frame (a couple of mascot lines, cut at the PTY output-queue boundary) and later frames only partially landed.

**Reproduction (PTY sim):** a standalone program that dups one pipe onto fds 1 and 2 (one shared OFD, like a terminal), runs `teeStderr`'s exact setup, then `fcntl(F_GETFL)` on fd 1 → reported `O_NONBLOCK` set (the poison). End-to-end with `creack/pty` (which dups the slave onto the child's 0/1/2): the known-good binary at f75c874 rendered ~9.5KB first frame (full mascot + bubbles + status bar); the HEAD binary rendered ~1–2KB (truncated). Verified against `bugs.md`'s original reporter symptom.

**Fix:** the terminal echo now uses a **fresh, dedicated open file description** instead of mutating the shared stderr/stdout OFD. `echoFd(saved)`: when saved stderr is a TTY (`isTty` via `TIOCGETA`), open `/dev/tty` with `O_WRONLY|O_NONBLOCK|O_CLOEXEC` — a new `open()` gets its own file-status flags, so its `O_NONBLOCK` cannot leak onto fd 1. If `/dev/tty` is unavailable it returns -1 (echo disabled; the crash log still receives every byte) rather than risk a blocking write to a stalled terminal. When saved stderr is NOT a TTY (redirected file/pipe — the case that cannot produce a stalled-TTY stall), it echoes to the saved descriptor unchanged, preserving the redirect target and leaving its blocking mode untouched. The deadlock guarantee of d7c129d is preserved: the drain goroutine still never blocks on a terminal write.

**Tests (RED→GREEN):** new `internal/app/crash_log_stdout_regression_test.go` — `TestTeeStderr_DoesNotPoisonStdout` re-execs the test binary as a helper with fds 1 and 2 wired to a single shared OFD (one pipe, exactly like a terminal session), runs the real `teeStderr`, and asserts fd 1 stays blocking (fd-3 report). Fails RED with `STDOUT_NONBLOCKING=1` on the buggy code, passes GREEN with the fix. Existing `TestTeeStderr_GatedWhileTUIOwnsScreen` updated behavior preserved (non-TTY saved stderr → echo to the pipe). Full gate green: `go vet`, `go test -count=1 -race ./...` (87 packages, exit 0), gocognit/gocyclo clean, gofmt clean. End-to-end PTY sim of the fixed binary renders the complete TUI (mascot + all bubbles + status bar).
