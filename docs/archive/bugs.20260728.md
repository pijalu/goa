...
# Bug archive — 2026-07-28

All items from the 2026-07-28 session (TODO list) plus the
previously accumulated TO ARCHIVE list, closed and verified.
See bugs.md for the reporting guidelines.

## Session 2026-07-28 — fixed bugs

## Bug A — Reported goal complete: no verification detail, no visible timeout

**Symptom:** the `◆ Reported goal complete` widget shows nearly nothing while the
done-gate runs the goal's `verifyCommand` (export goa-export-20260728-002505.zip:
"elapsed 89.5s" and no hint that `go test` was running, nor its output).

**Root cause (confirmed):** `updateComplete` → `RequestComplete` →
`runVerification` ran the verify command SYNCHRONOUSLY inside the goal tool
call (hard 2-minute timeout, invisible). On success the tool result was the
plain string `"Goal marked complete."` — and `renderGoalSummary`
JSON-unmarshals the output, fails on plain text, and returned "" — so the
result bubble rendered EMPTY. Command, output and timeout surfaced nowhere.

**Status: FIXED.**
- **Live announcement:** the goal tool is now an `agentic.ContextResultTool`
  (NEW interface — ctx + StopTurn; without it a ContextTool would silently
  drop StopTurn). When the done-gate's confirming call is about to run the
  verify command, the tool emits an execution-progress update:
  `Running goal verification (timeout 2m0s):\n$ <exact command>` — visible
  live in the goal widget instead of a silent 89.5s.
- **Result evidence:** `GoalMode.RequestComplete` now returns
  `CompleteResult.Verification` (command, output, DurationMs, TimeoutMs) on
  success AND failure; `CommandVerifier.Verify` returns a rich
  `goal.VerifyOutcome`. A closed goal returns:
  `Goal marked complete.\n\nVerification passed in 12.3s (timeout 2m0s):\n$ <command>\n<output tail>`.
- **Clear timeout:** configurable `goals.verify_timeout` (default 2m), shown
  in BOTH the live announcement and the evidence block; still enforced
  (command killed at the deadline, marked timed out, fails verification).
- **Renderer:** `renderGoalSummary` passes plain-text results through
  unchanged instead of "" (also fixes "Goal blocked/paused/resumed" bubbles).

**Tests (all green):** `TestGoalTool_Complete_AnnouncesAndReportsVerification`
(progress has command+timeout; result has evidence block; StopTurn kept),
`TestGoalTool_Complete_NoEvidenceWithoutCommand`,
`TestGoalTool_Complete_VerifyFailureShowsOutput`,
`TestRequestComplete_VerifyCommandPass` (+evidence assertions, core/goal),
`TestRenderGoalSummary_PlainTextPassthrough` (tui/goal),
`TestGoalsVerifyTimeoutOr` (config parse),
`TestExecCommandVerifier_TimeoutEnforced` / `_DefaultTimeout` /
`_ExitCodePropagates` (internal/app).

---

## Bug B — Read stuck (55.1s × 2 parallel reads) — root cause: blocking LSP spawn

**Symptom:** 2 parallel `read` calls (`tools/convert_compat_test.py`,
`tools/convert_compat_json.py`) wedged ~55s each (identical "Took 55.1s");
user had to ESC (export goa-export-20260728-001829.zip).

**Root cause (CONFIRMED via export + code + env):** agent.log shows the calls at
00:17:22.502 then TOTAL SILENCE until ESC at 00:18:15.634. All earlier `.go`
reads in the session were instant. These were the FIRST `.py` reads:
`ReadFileTool.touchLSP` → `Manager.DidChange(context.Background())` →
`clientFor` SYNCHRONOUSLY spawns the language server (single-flight: the second
read waits on `wg.Wait()` — hence identical durations). `pyright` is not on PATH
(verified), so `resolveCommand` fell back to npx, which downloaded the package
on first run (~55s); `client.Initialize` ran with `context.Background()` (no
timeout — a hung server wedges the read FOREVER, cf. Issue 21's 6h wedge). Not
a race condition: a blocking single-flight spawn.

**Status: FIXED** — subsumed by Issue LSP below (async non-blocking spawn +
whole-integration gating). Regression test:
`internal/lsp/manager_async_test.go TestManager_TouchNeverBlocksOnSpawn` (8
concurrent touches on a hanging spawn all return < 2s, exactly 1 spawn kicked).

---

## Issue LSP — LSP integration does not behave as expected (dedicated)

**User directives:** (1) "LSP is currently off — but when enabled, it should
support *all* LSP supported files!" (2) "make sure to include actual LSP tests
for js, py and go files: edit/write should be wired and provide correct hints".

**Gaps (all confirmed):**
1. **Off is not off.** `tools.enabled.lsp: false` removed only the `lsp` tool;
   the manager (read/edit/write touch + diagnostics) kept running, spawning
   servers/downloads in the background (caused Bug B).
2. **Touches block tool execution.** `clientFor` spawned synchronously with no
   timeout; single-flight parked concurrent tools (Bug B: 55s; potentially ∞).
3. **`.go`-only wiring.** `editfile.go:321` / `writefile.go:131` guarded
   `strings.HasSuffix(path, ".go")` — py/js/ts/rs/... files got NO diagnostics
   even with LSP enabled; `read` touched everything (inconsistent).
4. **Wrong diagnostics label.** `formatLSPDiagnostics` hardcoded
   "Diagnostics (gopls):" for every server.

**Status: FIXED** — plus FOUR more latent bugs found by the live tests:

1. **Manager async spawn** (`internal/lsp/manager.go`): `clientFor` is now
   non-blocking (kicks a background spawn via channel-flight, returns nil
   "starting"); `waitClientFor` (query ops: definition/references/hover/
   symbols) waits bounded by the caller ctx. Handshake (resolve+Initialize)
   bounded by `spawnHandshakeTimeout` (3m, covers cold npx downloads); the
   server process stays on the lifecycle ctx so a healthy server is never
   killed by that deadline. `Server.Close` got a 2s internal shutdown bound —
   a wedged server used to hang Close (and the spawn flight) forever.
2. **Whole-integration gating:** `newLSPManager` returns nil when
   `!cfg.Tools.Enabled.LSP` (in addition to top-level `lsp: false`) — off
   means off. `/tools:lsp:on` creates the manager live and wires read/edit/
   write (`makeLSPToolRuntime`); `/tools:lsp:off` unwires and closes it (new
   `core.Context.ToolTeardown` hook).
3. **All-file wiring:** `.go` guards removed in edit/write; the tool consults
   `Manager.ServerIDFor(path)` (new) and the manager selects per extension.
4. **Correct label:** diagnostics block names the actual server
   (`ServerIDFor`, fallback "lsp").
5. **Found by live tests (would have made py/js unusable):**
   - `ServerCapabilities` provider flags were strict `bool`; pyright sends
     `{"workDoneProgress":true}` (spec allows bool|object) → EVERY pyright
     initialize failed → spawn marked broken. Now `any`.
   - `Diagnostic.Code` was strict `string`; tsserver sends numeric codes
     (2552) → handler dropped ALL tsserver diagnostics. Now `any`.
   - `uriFor` now resolves symlinks: tsserver canonicalizes URIs
     (/var→/private/var on macOS) while gopls/pyright echo the didOpen URI —
     resolving up front keeps both consistent.
   - Initialize now declares `textDocument.publishDiagnostics` client
     capability — tsserver only pushes diagnostics when declared.
   - npx fallback used bare `npx <pkg>`: invalid for pyright ("Unexpected
     option --stdio" — the langserver bin lives INSIDE the pyright package)
     and tsserver (needs `typescript@5` — v6+ dropped lib/tsserver.js).
     `NpxSpec` gained `binary` + `extra_packages`; resolve builds
     `npx --yes --package <pkg> [--package <extra>] <bin> <args>`; registry
     fixed for pyright/typescript/svelte/vue/astro/prisma/dockerfile/biome.

**Tests (all green, -race):**
- manager: `TestManager_TouchNeverBlocksOnSpawn` (Bug B regression),
  `TestManager_WaitClientForSpawn` (ctx-bounded wait + self-heal),
  `TestManager_SpawnHandshakeTimeout` (wedged server marked broken ~100ms);
  concurrent + integration suites updated for async spawn.
- client: `TestClient_Initialize_ObjectProviderCapabilities`.
- registry: `TestResolveCommand_NpxForm`, `TestResolveCommand_NpxExtraPackages`,
  `TestRegistry_NpxBinaries` (pins all bin≠pkg specs + typescript@5).
- tools: `TestWriteFileTool_LSPManager_NotifyPython`,
  `TestEditFileTool_LSPManager_NotifyJavaScript` (all-file wiring + labels),
  `*_SkipsUnsupported` (manager-side selection).
- bootstrap: `TestNewLSPManager_ToolFlagDisable`,
  `TestLSPToggleOn_CreatesManagerAndWiresFileTools`,
  `TestLSPToggleOff_UnwiresAndCloses`, `TestLSPToggleOn_GloballyDisabled`.
- **LIVE integration (real servers, tools/lsp_live_integration_test.go):**
  `TestLiveLSP_Go` (gopls: "undefined: undefinedXYZ"),
  `TestLiveLSP_Python` (pyright via npx: '"undefined_xyz" is not defined'),
  `TestLiveLSP_JavaScript` (tsserver via npx+typescript@5: "Cannot find name
  'undefinedXYZ'"), `TestLiveLSP_EditPython` (edit path). Verified on this
  machine: go 0.3s, py 3.3s, js 3.4s, edit-py 4.3s.

---

## Bug C — Headless: ask_user_question must be disabled

**Symptom:** in headless mode (`goa -p ...`, `--goal`, `--orchestrate`) the
`ask_user_question` tool is registered, but there is no user to answer.

**Root cause:** `registerOptionalTools` (bootstrap.go:603) registers
`ask.AskUserQuestionTool{}` whenever `!cfg.Tools.Enabled.ClarifyDisabled`,
regardless of headless. The Clarify callback is only attached in interactive
`App.Run` (`attachClarifyTool`), so in headless the tool is present but
host-less.

**Status: FIXED.** The headless flag is threaded from `RuntimeOptions.Headless()`
through `InitSubsystems` → `initBaseSubsystems` → `registerTools` →
`registerOptionalTools`, which skips `ask_user_question` when headless.
Defense in depth confirmed: the tool already returns a clear `no_host`
ToolError ("no clarify handler configured (running headless?)") when invoked
without a Clarify callback.

**Tests (green):** `TestRegisterTools_HeadlessSkipsAskUser` (headless → not
registered; interactive → registered by default). Pre-existing
`TestRegisterTools_ClarifyDefaultOn` / `_ClarifyDisabled` still green.

---

## Bug D — Read widget: last line keeps pending bg on success (above spinner)

**Symptom:** a read tool call rendered as the LAST chat entry starts blue
(3 lines, filename in the middle). On success only the first 2 lines turn
green; the last line stays blue.

**Root cause (CONFIRMED, reproduced end-to-end):** the compositor's
`renderDiff` skip window. `repaintWindow` skips the last `lastScrollCount`
transcript rows ("already written by the scroll"), but the count was computed
as `max(0, target - c.vt)` — the window-top MOVE — while the scroll actually
wrote `target - max(c.vt, c.scrollTop)` rows. When the canvas SHRINKS then
regrows (assistant stream finalizing mid-flip: vtop 11→10→11 with the
scrollback watermark pinned at 11), the scroll wrote ZERO rows yet
`lastScrollCount` was 1 — so the LAST transcript row (the bottom tool
widget's last line, sitting directly above the spinner) was skipped and kept
its stale pending background. Reproduced: TWO parallel reads at the bottom of
a scrolled chat — the second widget's bottom pad stayed `tool_pending_bg`
while its header/Took rows flipped to `tool_success_bg`. The viewport cache
was all-green; the compositor's byte stream skipped exactly that row.

**Status: FIXED.** `tui/compositor.go renderDiff`:
`lastScrollCount = max(0, target - max(c.vt, c.scrollTop))` — the count now
always equals the rows the scroll actually advanced (identical in genuine
scroll frames; zero when the watermark didn't move). **This is also the
long-standing Issue 20 root cause** (tool widget's last line losing its
status bg above the spinner — same mechanism, same fix).

**Tests (RED→GREEN, -race):**
`TestUI_ToolWidgetRowsKeepStatusBgAfterStreamShrink` (internal/app — two
reads + stream-shrink mid-flip: asserted every row leaves the pending bg;
was RED with row 8 stale-blue, now GREEN),
`TestUI_ReadWidgetAllRowsKeepStatusBg` (baseline flip), Issue 20's
`TestCompositor_ToolWidgetLastRowKeepsStatusBg` still green, full tui + app
suites green with -race.

---

## Bug E — Empty tool widget (blank header + body, only "elapsed" visible)

**Symptom (export goa-export-20260728-001829.zip):** during a 3-way parallel
bash batch a THIRD tool box rendered completely empty except its ticking
"elapsed 88.8s" line.

**Root cause (CONFIRMED by code; deltas are not persisted —
`sessionstore.go:258` — so the export cannot show them):**
`Agent.handleToolCallPartial` emitted `EventToolCall{IsDelta:true}` as soon as
a tool-call START chunk arrived — possibly with `ToolName == ""` (OpenAI-style
streams ship id/index first, name in a later chunk). The tracker then either
(1) created a widget with EMPTY name that later named deltas could not match
(`findStreamingNoID("bash")` ≠ `ToolName()==""`) → a SECOND widget was created
for the real call and the blank one orphaned, or (2) registered the nameless
widget under the call id — later deltas updated its args but **the widget has
no SetToolName**, so its header stayed blank forever.

**Status: FIXED.**
- `agent_streaming.go` (`handleToolCallPartial` + `handleToolCallDeltaByIndex`):
  no delta is emitted while `emitName == ""`; args accumulate and are re-emitted
  cumulative, so the first named delta carries the full prefix (no display loss).
- `tooltracker.onCallDelta` (defense in depth): nameless deltas are ignored;
  the call gets its widget at its first named delta or the final event.

**Tests (all green):** `TestAgent_NamelessToolCallDeltaNotEmitted` (agent
guard), `TestTracker_NamelessDeltaCreatesNoWidget` (tracker guard),
`TestUI_NamelessToolCallDeltaNoBlankWidget` (uiScenario: id-first stream →
exactly one widget, never a blank box).

---

## Bug W — Queued/serialized tool calls display fake "elapsed"; add ⧖ "waiting Ns…"

**Symptom (user directive):** when multiple tool calls execute and some are
blocked (scheduler conflict / MaxParallel / Collect-held results), the blocked
widgets show a ticking "elapsed" as if executing. "Make it clear on the tool
call UI — with a 'waiting xx seconds...' — elapsed should be only shown on tool
being executed." Icon for waiting: **⧖** (hourglass), not the dots.

**Root cause (confirmed):** `internal/app/stats.go:615` stamped
`tc.SetStatus(tui.ToolRunning)` on EVERY final tool-call event, contradicting
the tracker design (`finalize` deliberately does NOT pre-stamp Running). And
the scheduler emitted no event when a queued task actually started, so the UI
could not distinguish queued vs executing (export 001829: three bash widgets
all showing the same "elapsed 88.8s").

**Status: FIXED.**
- `ToolScheduler.OnStart(callID)` (new hook) fires in `start()` for immediate
  AND unblocked-after-queue tasks; the agent emits the new
  `EventToolStart{ToolName, ToolCallID}` from it (persisted like tool_progress
  for export diagnostics; ignored safely by headless/ACP/orchestrator views).
- App: `handleToolCall` no longer stamps Running on final calls; new
  `handleToolStart` → `Tracker.OnStart` flips Pending→Running (true execution
  start — `SetStatus` restarts startTime, so elapsed measures execution only).
  Progress events keep their Pending→Running backstop; `failPendingTools`
  remains the straggler net.
- Widget: Pending + args complete → **⧖** hourglass + "waiting Ns…" counted
  from args-complete (`markArgsComplete` centralizes the stamp across
  SetArgsComplete/SetArgsJSON/AddToolExecution); Pending + args streaming →
  unchanged ◉; Running → ● + "elapsed"; Success/Error → "Took".
  `patchRunningToolWidgets` patches waiting widgets too (`isLiveToolWidget`),
  so "waiting" ticks live on spinner frames.

**Tests (all green):** `TestToolScheduler_OnStart` (immediate + unblocked
order), `TestAgent_EmitsToolStartBeforeResult`, `TestTracker_OnStartFlips
PendingToRunning`, `TestToolExecution_WaitingPresentation` (⧖/waiting vs ◉/
elapsed), `TestUI_QueuedToolsShowWaiting` (3-call batch: queued show ⧖ waiting,
only the started one Running). Existing Running-stamp tests updated to the new
lifecycle (`TestHandleToolCall_IsDeltaThenFinal_TransitionsToRunning`,
`TestHandleToolCall_NonDeltaCreatesWaitingWidget`).

---

## Bug Docs — Documentation must match latest developments

**Directive:** "Documentation should be updated to match latest devs".

**Status: FIXED.** Changes:
- **`tools/lsp.go` schema description** — was "Query the Go language server
  (gopls)… Go source file path"; now multi-language (manager selects the
  server per file), and the unavailable error points at `/tools:lsp:on`.
- **`docs/TOOLS.md`** — new `lsp` tool entry (ops, per-file server selection,
  diagnostics-on-edit/write for ALL supported files with correct labels,
  non-blocking guarantee, config incl. `/tools:lsp:on|off` live toggle and
  `lsp: false` / `lsp.servers` / `lsp.disable_download`); `ask_user_question`
  documents the headless exclusion.
- **`docs/CONFIGURATION.md`** — new top-level `lsp:` section (master-switch
  semantics of `tools.enabled.lsp`, async spawn, per-server overrides).
- **`docs/GOALS.md`** — `goals.verify_timeout` (default 2m) replaces the
  hardcoded "2-minute hard timeout"; documents the live verification
  announcement + completion evidence block.
- **`docs/TUI.md`** — new "Tool widget lifecycle" section (◉ streaming /
  ⧖ waiting / ● running / ✓✗ terminal, EventToolStart semantics).

**Validation:** `go test ./docs/...` green; lsp YAML block structure-checked;
remaining `gopls`-only mentions grep-verified gone from docs (archive
excluded).
...
**RESOLUTION (2026-07-28): CLOSED — root cause found and fixed (see Bug D in
TODO, now archived).** The byte-level investigation above missed it because no
scenario combined all three required ingredients: (1) TWO stacked tool widgets
at the bottom, (2) an assistant stream finalizing mid-flip (canvas SHRINK then
regrow → vtop bounces 11→10→11), (3) the scrollback watermark pinned (no
scroll on the flip frame). Under exactly those conditions the compositor's
`renderDiff` computed `lastScrollCount = target - c.vt = 1` although the
scroll wrote 0 rows, and `repaintWindow` skipped the LAST transcript row —
the widget's last line — leaving the stale pending bg. Fixed in
`tui/compositor.go` (`lastScrollCount = target - max(c.vt, c.scrollTop)`);
regression tests `TestUI_ToolWidgetRowsKeepStatusBgAfterStreamShrink` and
`TestUI_ReadWidgetAllRowsKeepStatusBg` (internal/app) prove RED→GREEN.
...
**Status: CLOSED (2026-07-28) — root cause confirmed as the blocking LSP
spawn; mechanism eliminated.** The original analysis ruled LSP out on the
false assumption that `tools.enabled.lsp: false` disabled the manager — it
did NOT (only the top-level `lsp: false` did, see Issue LSP). Verified from
the export: config carries `tools.enabled.lsp: false` and the wedged call was
`read tools/convert_compat_json.py:150-220` — a PYTHON file whose first touch
spawns pyright synchronously (cold npx download + unbounded Initialize at the
time). The read goroutine parked ~6h inside `clientFor`'s single-flight; the
turn's `Collect()` waited on it (agent.log silent, no further HTTP requests);
ESC cancelled the manager lifecycle, aborting the spawn, so the read
completed and its result landed during the unwind ("Took 21249.6s"). The
identical 55s wedge recurred on the same files two hours later (Bug B,
export 001829). The fix (Issue LSP) eliminates the mechanism by construction:
touches never block (async spawn), the handshake is bounded (3m), and
`tools.enabled.lsp: false` now removes the manager entirely — regression
`TestManager_TouchNeverBlocksOnSpawn`. The goroutine-dump/export hooks remain
for any future DISTINCT wedge; reopen only with a dump showing a different
cause.
