...
**Status: all three items FIXED, validated, and committed (2026-08-21).**
The closed bug reports live alongside this plan in
`docs/archive/bugs-20260821-delegate-schedule-scrolloff.md`.

Deviations from the initial handover discovered while executing (byte-path
tracing, hard rule #2):
- `Orchestrator.Emit(role, "main", …)` would have been **swallowed**: it
  produces `Kind: "content"`, and both the TUI forwarder and the headless
  consumer only handle `To: stream_start/stream_chunk/stream_end` for that
  kind. The visible-failure emit therefore uses the new
  `ForegroundOrchestrator.EmitMessage` (Kind `"message"`), which routes
  through the forwarder's default branch → InterAgent chat event →
  `AddAgentMessage` (TUI) / `CompanionChunk` (headless).
- Commits landed on `feature/team` (the checked-out working branch holding
  all delegate_to-async infrastructure the fixes build on), not `main`.
