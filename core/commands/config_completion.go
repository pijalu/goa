...
func configKeyCompletions(prefix string) []core.ArgCompletion {
	keys := []struct{ value, description string }{
		...
		{"execution.retries", "integer (default 5)"},
		...
		{"execution.loop_auto_resume", "true | false (resume agent after loop stop)"},
		{"execution.loop_auto_resume_message", "message sent on loop auto-resume"},
