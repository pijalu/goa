...
var goalNextOptions = []struct{ value, desc string }{{"first", "queue at the front — runs right after the active goal (default)"}, {"last", "queue at the end — runs after all queued goals"}, {"fresh", "queue on a clean context"}, {"reuse", "queue reusing the current conversation"}, {"rfirst", "reuse the conversation, queue at the front"}, {"rlast", "reuse the conversation, queue at the end"}}
