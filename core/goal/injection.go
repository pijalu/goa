...
func BuildStaticGoalReminder(snapshot GoalSnapshot) string {
	...
	b.WriteString("prevents progress, or the objective cannot be completed as stated, call goal with action \n")
	b.WriteString("`update`, status `blocked`. Otherwise keep working — after your turn ends you will be prompted to continue. \n")
	b.WriteString("Call goal with action `update` as soon as the goal is genuinely done or cannot proceed; don't keep going \n")
...
func BuildPausedNote(snapshot GoalSnapshot) string {
	...
	b.WriteString("asks you to continue that goal. If the user does ask you to work on it, call goal with action ")
	b.WriteString("`update`, status `active` before resuming goal-driven work. The user can also resume it with ")
