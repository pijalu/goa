...
func assertGraphLines(t *testing.T, lines []string) {
	t.Helper()
	joined := strings.Join(lines, "\n")
	plain := ansi.Strip(joined)
	for _, want := range []string{"■ /proj/a 53%", "■ /proj/b 43%", "■ /proj/c 4%"} {
		if !strings.Contains(plain, want) {
			t.Errorf("legend entry %q missing or mangled", want)
		}
	}
	lastLegend, heading := -1, -1
	for index, line := range lines {
		plainLine := ansi.Strip(line)
		if strings.Count(plainLine, "■") > 1 {
			t.Errorf("multiple legend entries collapsed: %q", plainLine)
		}
		if strings.Contains(plainLine, "/proj/c 4%") {
			lastLegend = index
		}
		if strings.Contains(plainLine, "By provider") {
			heading = index
		}
	}
	if lastLegend < 0 || heading < 0 {
		t.Fatalf("legend or heading not found:\n%s", plain)
	}
	if heading != lastLegend+2 || ansi.Strip(lines[lastLegend+1]) != "" {
		t.Errorf("expected one empty line between legend and heading")
	}
...
func TestMDStreamRenderer_GraphLinesStayStandalone(t *testing.T) {
	...
	assertGraphLines(t, r.Render(input))
}
